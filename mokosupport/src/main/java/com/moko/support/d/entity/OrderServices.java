package com.moko.support.d.entity;

import java.util.UUID;

public enum OrderServices {
    SERVICE_DEVICE_INFO(UUID.fromString("0000180A-0000-1000-8000-00805F9B34FB")),
    SERVICE_CUSTOM(UUID.fromString("0000AA00-0000-1000-8000-00805F9B34FB")),
    SERVICE_ADV_TRIGGER(UUID.fromString("0000FEE0-0000-1000-8000-00805F9B34FB")),
    SERVICE_ADV_UID(UUID.fromString("0000FEAA-0000-1000-8000-00805F9B34FB")),
    SERVICE_ADV_IBEACON(UUID.fromString("0000FEAB-0000-1000-8000-00805F9B34FB")),
    SERVICE_ADV_DEVICE(UUID.fromString("0000EA00-0000-1000-8000-00805F9B34FB")),
    SERVICE_OTA(UUID.fromString("00001530-1212-EFDE-1523-785feabcd123")),
    SERVICE_ADV_OTA(UUID.fromString("0000AEFF-0000-1000-8000-00805F9B34FB")),
    ;
    private UUID uuid;

    OrderServices(UUID uuid) {
        this.uuid = uuid;
    }

    public UUID getUuid() {
        return uuid;
    }
}
