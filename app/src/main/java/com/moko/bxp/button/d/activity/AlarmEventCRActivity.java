...
import com.moko.bxp.button.d.databinding.DActivityAlarmEventCrBinding;
...
public class AlarmEventCRActivity extends BaseActivity {


    private DActivityAlarmEventCrBinding mBind;

    @Override
    protected void onCreate(Bundle savedInstanceState) {
        super.onCreate(savedInstanceState);
        mBind = DActivityAlarmEventCrBinding.inflate(getLayoutInflater());
        ...
        runOnUiThread(new Runnable() {
            ...
            public void run() {
                if (MokoConstants.ACTION_DISCONNECTED.equals(action)) {
                    // 设备断开，通知页面更新
                    AlarmEventCRActivity.this.finish();
                }
            }
        });
    }
                            ...
                            if (flag == 0x01 && length == 0x01) {
                                ...
                                switch (configKeyEnum) {
                                    case KEY_SINGLE_PRESS_EVENT_CLEAR:
                                        if (result == 0) {
                                            ToastUtils.showToast(AlarmEventCRActivity.this, "Opps！Save failed. Please check the input characters and try again.");
                                        } else {
                                            ToastUtils.showToast(AlarmEventCRActivity.this, "Success！");
    ...
    public void onExportAlarmEvent(View view) {
        if (isWindowLocked())
            return;
        Intent intent = new Intent(this, ExportLongConnectionDataActivity.class);
        startActivity(intent);
    }
}
